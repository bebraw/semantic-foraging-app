...
- The primary search affordance may include example queries when those examples clarify supported semantic result shapes, but they should stay behind a compact on-demand control that sits inline with the main search actions instead of consuming extra search-surface height by default.
