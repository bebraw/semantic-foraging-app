...
describe("createHealthPayload", () => {
  it("builds the stable payload shape for health checks", () => {
    expect(
      createHealthPayload("vibe-template-worker", ["/", "/api/health", "/api/intent", "/api/intent/clarify", "/api/explanation"]),
    ).toEqual({
      ok: true,
...
describe("createHealthResponse", () => {
  it("returns the stable JSON payload for health checks", async () => {
    const response = createHealthResponse(["/", "/api/health", "/api/intent", "/api/intent/clarify", "/api/explanation"]);
