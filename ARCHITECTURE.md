...
- Keep server-rendered search surfaces query-shaped: typed screen models should expose why a result maps to a map, cards, table, or prose instead of burying that choice in view-only logic.

## Tooling Baseline

- Local development and local CI target macOS as the supported host platform baseline.
- Node is pinned through `package.json`, and the supported npm range is declared there.
